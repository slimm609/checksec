...
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f1) != "Full RELRO" ]]; then
...
for bin in partial partial32 partial_cl partial_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f1) != "Partial RELRO" ]]; then
...
for bin in none none32 none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f1) != "No RELRO" ]]; then
...
for bin in rel.o rel32.o rel_cl.o rel_cl32.o; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f1) != "N/A" ]]; then
...
# Canary found
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f2) != "Canary found" ]]; then
...
# No Canary found
for bin in none none32 none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f2) != "No Canary found" ]]; then
...
# NX enabled
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f3) != "NX enabled" ]]; then
...
# NX disabled
for bin in none none32 none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f3) != "NX disabled" ]]; then
...
# N/A
for bin in rel.o rel32.o rel_cl.o rel_cl32.o; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f3) != "N/A" ]]; then
...
# PIE enabled
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f4) != "PIE enabled" ]]; then
...
# No PIE
for bin in none none32 none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f4) != "No PIE" ]]; then
...
for bin in dso.so dso32.so dso_cl.so dso_cl32.so; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f4) != "DSO" ]]; then
...
# REL
for bin in rel.o rel32.o rel_cl.o rel_cl32.o; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f4) != "REL" ]]; then
...
for bin in cfi cfi32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --extended --format=csv | cut -d, -f5) != "with CFI" ]]; then
...
for bin in none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --extended --format=csv | cut -d, -f5) != "without CFI" ]]; then
...
for bin in sstack sstack32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --extended --format=csv | cut -d, -f6) != "with SafeStack" ]]; then
...
# without SafeStack
for bin in none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --extended --format=csv | cut -d, -f6) != "without SafeStack" ]]; then
...
# No RPATH
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f5) != "No RPATH" ]]; then
...
for bin in rpath rpath32 rpath_cl rpath_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f5) != "RPATH" ]]; then
...
# N/A
for bin in rel.o rel32.o rel_cl.o rel_cl32.o; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f5) != "N/A" ]]; then
...
# No RUNPATH
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f6) != "No RUNPATH" ]]; then
...
for bin in runpath runpath32 runpath_cl runpath_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f6) != "RUNPATH" ]]; then
...
# N/A
for bin in rel.o rel32.o rel_cl.o rel_cl32.o; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f6) != "N/A" ]]; then
...
# No Symbols
for bin in all all32 all_cl all_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f7) != "No Symbols" ]]; then
...
# Symbols
for bin in none none32 none_cl none_cl32; do
  if [[ $("${PARENT}"/checksec --file="${DIR}/binaries/${bin}" --format=csv | cut -d, -f7) != "Symbols" ]]; then
